...
{
    // Check if it's a URL format (postgres://user:pass@host:port/db)
    if (databaseUrl.Contains("://"))
    {
        ...
        {
            // If URL parsing fails, maybe it's actually a connection string that happens to have :// (unlikely but safe)
            connectionString = databaseUrl;
        }
    }
    else
    {
        // It's already in Host=... format
        connectionString = databaseUrl;
    }
}
...
else
{
    ...
    var user = Environment.GetEnvironmentVariable("DB_USER")?.Trim('"');
    var pass = Environment.GetEnvironmentVariable("DB_PASSWORD")?.Trim('"');
    var db = Environment.GetEnvironmentVariable("DB_NAME")?.Trim('"');
    var port = Environment.GetEnvironmentVariable("DB_PORT")?.Trim('"') ?? "5432";
